...
import org.talend.dataprep.transformation.pipeline.util.PipelineConsoleDump;
import org.talend.dataprep.transformation.pipeline.util.StepNodeTransformer;

public class Pipeline implements Node, Serializable {
    ...
    public static class Builder {
        ...
        public Builder withInput(String url) {
            this.url = url;
            return this;
        }
        ...
        public Pipeline build() {
            ...
            if (inFilter != null) {
                current = NodeBuilder.filteredSource(inFilter, url);
            } else {
                current = NodeBuilder.source(url);
            }
            ...
            if (outFilter != null) {
                ...
                current.to(new FilterNode((row, metadata) -> {
                    final Predicate<DataSetRow> predicate = outFilter.apply(metadata);
            ...
            if (limit != null && limit > 0) {
                current.to(new LimitNode(limit)); // limit the output
            }
