...
public class ZipLinkTest {
    ...
    public void should_emit_single_input_row_when_all_source_has_emitted_one() {
        ...
        final ZipLink zipLink = new ZipLink(new Node[]{source1, source2}, target);

        final DataSetRow row = new DataSetRow(new HashMap<>());
        final RowMetadata metadata = new RowMetadata(new ArrayList<>());

        // when
        source1.accept(new RuntimeNodeVisitor()).receive(row, metadata);
        ...
        // when
        source2.accept(new RuntimeNodeVisitor()).receive(row, metadata);
    ...
    public void should_emit_signal_when_all_source_has_emitted_one() {
        ...
        // when
        source1.accept(new RuntimeNodeVisitor()).signal(END_OF_STREAM);

        // then
        assertThat(target.getReceivedSignals(), hasSize(0));

        // when
        source2.accept(new RuntimeNodeVisitor()).signal(END_OF_STREAM);
